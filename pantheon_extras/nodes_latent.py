...
import pantheon_extras.nodes_post_processing
import torch


def reshape_latent_to(target_shape, latent, repeat_batch=True):
    if latent.shape[1:] != target_shape[1:]:
        latent = pantheon.utils.common_upscale(latent, target_shape[-1], target_shape[-2], "bilinear", "center")
    if repeat_batch:
        return pantheon.utils.repeat_to_batch_size(latent, target_shape[0])
    else:
        return latent
...
class LatentMultiply:
    ...
    def op(self, samples, multiplier: float = 1.0):
...
class LatentInterpolate:
    ...
    def op(self, samples1, samples2, ratio: float = 1.0):
        samples_out = samples1.copy()
...
class LatentOperationSharpen:
    ...
    def op(self, sharpen_radius, sigma, alpha):
        def sharpen(latent, **kwargs):
            ...
            kernel = pantheon_extras.nodes_post_processing.gaussian_kernel(kernel_size, sigma, device=luminance.device)
