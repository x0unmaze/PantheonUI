...
class EmptyLTXVLatentVideo:
    ...
    def generate(self, width, height, length, batch_size=1):
        latent = torch.zeros([batch_size, 128, ((length - 1) // 8) + 1, height // 32, width // 32], device=pantheon.model_management.intermediate_device())
...
class LTXVImgToVideo:
    ...
    def generate(self, positive, negative, image, vae, width, height, length, batch_size, image_noise_scale):
        pixels = pantheon.utils.common_upscale(image.movedim(-1, 1), width, height, "bilinear", "center").movedim(1, -1)
