...
from pantheon.k_diffusion import sampling as k_diffusion_sampling
...
class BasicScheduler:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "model": ("MODEL",),
                "scheduler": (pantheon.samplers.SCHEDULER_NAMES, ),
    ...
    def get_sigmas(self, model, scheduler, steps: int = 20, denoise: float = 1.0):
        total_steps = steps
        if denoise < 1.0:
            if denoise <= 0.0:
        ...
        sigmas = pantheon.samplers.calculate_sigmas(model.get_model_object("model_sampling"), scheduler, total_steps).cpu()
...
class BetaSamplingScheduler:
    ...
    def get_sigmas(self, model, steps, alpha, beta):
        sigmas = pantheon.samplers.beta_scheduler(model.get_model_object("model_sampling"), steps, alpha=alpha, beta=beta)
        return (sigmas, )
...
class KSamplerSelect:
    ...
    def INPUT_TYPES(s):
        return {"required":
                    {"sampler_name": (pantheon.samplers.SAMPLER_NAMES, ),
                      }
               }
    ...
    def get_sampler(self, sampler_name):
        sampler = pantheon.samplers.sampler_object(sampler_name)
        return (sampler, )
...
class SamplerDPMPP_3M_SDE:
    ...
    def get_sampler(self, eta, s_noise, noise_device):
        ...
        sampler = pantheon.samplers.ksampler(sampler_name, {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerDPMPP_2M_SDE:
    ...
    def get_sampler(self, solver_type, eta, s_noise, noise_device):
        ...
        sampler = pantheon.samplers.ksampler(sampler_name, {"eta": eta, "s_noise": s_noise, "solver_type": solver_type})
        return (sampler, )
...
class SamplerDPMPP_SDE:
    ...
    def get_sampler(self, eta, s_noise, r, noise_device):
        ...
        sampler = pantheon.samplers.ksampler(sampler_name, {"eta": eta, "s_noise": s_noise, "r": r})
        return (sampler, )
...
class SamplerDPMPP_2S_Ancestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = pantheon.samplers.ksampler("dpmpp_2s_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerEulerAncestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = pantheon.samplers.ksampler("euler_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerEulerAncestralCFGPP:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = pantheon.samplers.ksampler(
...
class SamplerLMS:
    ...
    def get_sampler(self, order):
        sampler = pantheon.samplers.ksampler("lms", {"order": order})
        return (sampler, )
...
class Noise_RandomNoise:
    ...
    def generate_noise(self, input_latent):
        ...
        return pantheon.sample.prepare_noise(latent_image, self.seed, batch_inds)
...
class SamplerCustom:
    ...
    def sample(self, model, add_noise, noise_seed, cfg, positive, negative, sampler, sigmas, latent_image):
        ...
        samples = pantheon.sample.sample_custom(model, noise, cfg, sampler, sigmas, positive, negative, latent_image, noise_mask=noise_mask, callback=callback, disable_pbar=disable_pbar, seed=noise_seed)
...
class Guider_Basic(pantheon.samplers.CFGGuider):
...
class CFGGuider:
    ...
    def get_guider(self, model, positive, negative, cfg):
        guider = pantheon.samplers.CFGGuider(model)
        guider.set_conds(positive, negative)
        guider.set_cfg(cfg)
        return (guider,)

class Guider_DualCFG(pantheon.samplers.CFGGuider):
    ...
    def predict_noise(self, x, timestep, model_options={}, seed=None):
        ...
        out = pantheon.samplers.calc_cond_batch(self.inner_model, [negative_cond, middle_cond, self.conds.get("positive", None)], x, timestep, model_options)
        return pantheon.samplers.cfg_function(self.inner_model, out[1], out[0], self.cfg2, x, timestep, model_options=model_options, cond=middle_cond, uncond=negative_cond) + (out[2] - out[1]) * self.cfg1
...
class RandomNoise(DisableNoise):
    ...
    def get_noise(self, noise_seed: int = 0):
...
class SamplerCustomAdvanced:
    ...
    def sample(self, noise, guider, sampler, sigmas, latent_image):
        ...
        latent_image = latent["samples"]
        latent = latent.copy()
        latent_image = pantheon.sample.fix_empty_latent_channels(guider.model_patcher, latent_image)
        latent["samples"] = latent_image
        ...
        samples = samples.to(pantheon.model_management.intermediate_device())
