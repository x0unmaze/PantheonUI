...
import math
import struct
import pantheon.checkpoint_pickle
...
def load_torch_file(ckpt, safe_load=False, device=None):
    ...
    if ckpt.lower().endswith(".safetensors") or ckpt.lower().endswith(".sft"):
        ...
        if safe_load or ALWAYS_SAFE_LOAD:
            ...
            pl_sd = torch.load(ckpt, map_location=device, pickle_module=pantheon.checkpoint_pickle)
...
def get_attr(obj, attr: str):
    ...
    Important:
        Always prefer `pantheon.model_patcher.ModelPatcher.get_model_object` when
