...
import pantheon.float
import pantheon.model_management
import pantheon.lora
import pantheon.hooks
import pantheon.patcher_extension
from pantheon.patcher_extension import CallbacksMP, WrappersMP, PatcherInjection
from pantheon.pantheon_types import UnetWrapperFunction
...
def create_model_options_clone(orig_model_options: dict):
    return pantheon.patcher_extension.copy_nested_dicts(orig_model_options)
...
def wipe_lowvram_weight(m):
    if hasattr(m, "prev_pantheon_cast_weights"):
        m.pantheon_cast_weights = m.prev_pantheon_cast_weights
        del m.prev_pantheon_cast_weights
...
class LowVramPatch:
    ...
    def __call__(self, weight):
        ...
        if intermediate_dtype not in [torch.float32, torch.float16, torch.bfloat16]: #intermediate_dtype has to be one that is supported in math ops
            ...
            return pantheon.float.stochastic_rounding(pantheon.lora.calculate_weight(self.patches[self.key], weight.to(intermediate_dtype), self.key, intermediate_dtype=intermediate_dtype), weight.dtype, seed=string_to_seed(self.key))

        return pantheon.lora.calculate_weight(self.patches[self.key], weight, self.key, intermediate_dtype=intermediate_dtype)
...
def get_key_weight(model, key):
    ...
    if len(op_keys) < 2:
        weight = pantheon.utils.get_attr(model, key)
    else:
        op = pantheon.utils.get_attr(model, op_keys[0])
        try:
...
class ModelPatcher:
    def __init__(self, model, load_device, offload_device, size=0, weight_inplace_update=False):
        ...
        self.hook_patches: dict[pantheon.hooks._HookRef] = {}
        self.hook_patches_backup: dict[pantheon.hooks._HookRef] = None
        self.hook_backup: dict[str, tuple[torch.Tensor, torch.device]] = {}
        self.cached_hook_patches: dict[pantheon.hooks.HookGroup, dict[str, torch.Tensor]] = {}
        self.current_hooks: Optional[pantheon.hooks.HookGroup] = None
        self.forced_hooks: Optional[pantheon.hooks.HookGroup] = None  # NOTE: only used for CLIP at this time
        self.is_clip = False
        self.hook_mode = pantheon.hooks.EnumHookMode.MaxSpeed
    ...
    def model_size(self):
        if self.size > 0:
            return self.size
        self.size = pantheon.model_management.module_size(self.model)
        return self.size
    ...
    def get_model_object(self, name: str) -> torch.nn.Module:
        ...
        if name in self.object_patches:
            ...
            if name in self.object_patches_backup:
                ...
                return pantheon.utils.get_attr(self.model, name)
    ...
    def patch_weight_to_device(self, key, device_to=None, inplace_update=False):
        ...
        if device_to is not None:
            temp_weight = pantheon.model_management.cast_to_device(weight, device_to, torch.float32, copy=True)
        else:
            temp_weight = weight.to(torch.float32, copy=True)
        if convert_func is not None:
            temp_weight = convert_func(temp_weight, inplace=True)

        out_weight = pantheon.lora.calculate_weight(self.patches[key], temp_weight, key)
        if set_func is None:
            out_weight = pantheon.float.stochastic_rounding(out_weight, weight.dtype, seed=string_to_seed(key))
            if inplace_update:
                pantheon.utils.copy_to_param(self.model, key, out_weight)
            else:
                pantheon.utils.set_attr_param(self.model, key, out_weight)
        else:
        ...
        for n, m in self.model.named_modules():
            ...
            for name, param in m.named_parameters(recurse=True):
                if name not in params:
                    ...
                    break
            if not skip and (hasattr(m, "pantheon_cast_weights") or len(params) > 0):
                loading.append((pantheon.model_management.module_size(m), n, m, params))
        return loading
    ...
    def load(self, device_to=None, lowvram_model_memory=0, force_patch_weights=False, full_load=False):
        with self.use_ejected():
            ...
            for x in loading:
                ...
                if not full_load and hasattr(m, "pantheon_cast_weights"):
                    if mem_counter + module_mem >= lowvram_model_memory:
                        lowvram_weight = True
                        lowvram_counter += 1
                        if hasattr(m, "prev_pantheon_cast_weights"): #Already lowvramed
                            continue
                ...
                if lowvram_weight:
                    ...
                    m.prev_pantheon_cast_weights = m.pantheon_cast_weights
                    m.pantheon_cast_weights = True
                else:
                    if hasattr(m, "pantheon_cast_weights"):
                        if m.pantheon_cast_weights:
            ...
            for x in load_completely:
                ...
                m = x[2]
                params = x[3]
                if hasattr(m, "pantheon_patched_weights"):
                    if m.pantheon_patched_weights == True:
                        continue
                ...
                m.pantheon_patched_weights = True
    ...
    def patch_model(self, device_to=None, lowvram_model_memory=0, load_weights=True, force_patch_weights=False):
        with self.use_ejected():
            for k in self.object_patches:
                old = pantheon.utils.set_attr(self.model, k, self.object_patches[k])
        ...
        if unpatch_weights:
            ...
            for k in keys:
                ...
                if bk.inplace_update:
                    pantheon.utils.copy_to_param(self.model, k, bk.weight)
                else:
                    pantheon.utils.set_attr_param(self.model, k, bk.weight)
            ...
            for m in self.model.modules():
                if hasattr(m, "pantheon_patched_weights"):
                    del m.pantheon_patched_weights

        keys = list(self.object_patches_backup.keys())
        for k in keys:
            pantheon.utils.set_attr(self.model, k, self.object_patches_backup[k])
    ...
    def partially_unload(self, device_to, memory_to_free=0):
        with self.use_ejected():
            ...
            for unload in unload_list:
                ...
                lowvram_possible = hasattr(m, "pantheon_cast_weights")
                if hasattr(m, "pantheon_patched_weights") and m.pantheon_patched_weights == True:
                    move_weight = True
                    for param in params:
                        key = "{}.{}".format(n, param)
                        ...
                        if bk is not None:
                            ...
                            if bk.inplace_update:
                                pantheon.utils.copy_to_param(self.model, key, bk.weight)
                            else:
                                pantheon.utils.set_attr_param(self.model, key, bk.weight)
                            self.backup.pop(key)
                    ...
                    if move_weight:
                        ...
                        if lowvram_possible:
                            ...
                            m.prev_pantheon_cast_weights = m.pantheon_cast_weights
                            m.pantheon_cast_weights = True
                        m.pantheon_patched_weights = False
    ...
    def calculate_weight(self, patches, weight, key, intermediate_dtype=torch.float32):
        logging.warning("The ModelPatcher.calculate_weight function is deprecated, please use: pantheon.lora.calculate_weight instead")
        return pantheon.lora.calculate_weight(patches, weight, key, intermediate_dtype=intermediate_dtype)
    ...
    def set_hook_mode(self, hook_mode: pantheon.hooks.EnumHookMode):
        self.hook_mode = hook_mode

    def prepare_hook_patches_current_keyframe(self, t: torch.Tensor, hook_group: pantheon.hooks.HookGroup, model_options: dict[str]):
        curr_t = t[0]
    ...
    def register_all_hook_patches(self, hooks: pantheon.hooks.HookGroup, target_dict: dict[str], model_options: dict=None,
                                  registered: pantheon.hooks.HookGroup = None):
        self.restore_hook_patches()
        if registered is None:
            registered = pantheon.hooks.HookGroup()
        # handle WeightHooks
        weight_hooks_to_register: list[pantheon.hooks.WeightHook] = []
        for hook in hooks.get_type(pantheon.hooks.EnumHookType.Weight):
    ...
    def add_hook_patches(self, hook: pantheon.hooks.WeightHook, patches, strength_patch=1.0, strength_model=1.0):
        with self.use_ejected():
    ...
    def get_combined_hook_patches(self, hooks: pantheon.hooks.HookGroup):
    ...
    def apply_hooks(self, hooks: pantheon.hooks.HookGroup, transformer_options: dict=None, force_apply=False):
        ...
        if self.current_hooks == hooks and (not force_apply or (not self.is_clip and hooks is None)):
            return pantheon.hooks.create_transformer_options_from_hooks(self, hooks, transformer_options)
        self.patch_hooks(hooks=hooks)
        for callback in self.get_all_callbacks(CallbacksMP.ON_APPLY_HOOKS):
            callback(self, hooks)
        return pantheon.hooks.create_transformer_options_from_hooks(self, hooks, transformer_options)

    def patch_hooks(self, hooks: pantheon.hooks.HookGroup):
        with self.use_ejected():
            self.unpatch_hooks()
            if hooks is not None:
                model_sd_keys = list(self.model_state_dict().keys())
                memory_counter = None
                if self.hook_mode == pantheon.hooks.EnumHookMode.MaxSpeed:
                    # TODO: minimum_counter should have a minimum that conforms to loaded model requirements
                    memory_counter = MemoryCounter(initial=pantheon.model_management.get_free_memory(self.load_device),
                                                minimum=pantheon.model_management.minimum_inference_memory()*2)
    ...
    def patch_cached_hook_weights(self, cached_weights: dict, key: str, memory_counter: MemoryCounter):
        if key not in self.hook_backup:
            weight: torch.Tensor = pantheon.utils.get_attr(self.model, key)
        ...
        pantheon.utils.copy_to_param(self.model, key, cached_weights[key][0].to(device=cached_weights[key][1]))

    def clear_cached_hook_weights(self):
        self.cached_hook_patches.clear()
        self.patch_hooks(None)

    def patch_hook_weight_to_device(self, hooks: pantheon.hooks.HookGroup, combined_patches: dict, key: str, original_weights: dict, memory_counter: MemoryCounter):
        ...
        temp_weight = pantheon.model_management.cast_to_device(weight, weight.device, torch.float32, copy=True)
        if convert_func is not None:
            temp_weight = convert_func(temp_weight, inplace=True)

        out_weight = pantheon.lora.calculate_weight(combined_patches[key],
                                                 temp_weight,
    ...
    def unpatch_hooks(self) -> None:
        with self.use_ejected():
            ...
            for k in keys:
                pantheon.utils.copy_to_param(self.model, k, self.hook_backup[k][0].to(device=self.hook_backup[k][1]))
