...
def prepare_mask(noise_mask, shape, device):
    return pantheon.utils.reshape_mask(noise_mask, shape).to(device)
...
def get_hooks_from_cond(cond, full_hooks: pantheon.hooks.HookGroup):
    ...
    extra_hooks = pantheon.hooks.HookGroup.combine_all_hooks(hooks_list)
...
def get_additional_models_from_model_options(model_options: dict[str]=None):
    ...
    if model_options is not None and "registered_hooks" in model_options:
        registered: pantheon.hooks.HookGroup = model_options["registered_hooks"]
        for hook in registered.get_type(pantheon.hooks.EnumHookType.AdditionalModels):
            hook: pantheon.hooks.AdditionalModelsHook
...
def prepare_sampling(model: ModelPatcher, noise_shape, conds, model_options=None):
    ...
    pantheon.model_management.load_models_gpu([model] + models, memory_required=memory_required, minimum_memory_required=minimum_memory_required)
...
def prepare_model_patcher(model: 'ModelPatcher', conds, model_options: dict):
    ...
    hooks = pantheon.hooks.HookGroup()
    for k in conds:
        get_hooks_from_cond(conds[k], hooks)
    # add wrappers and callbacks from ModelPatcher to transformer_options
    model_options["transformer_options"]["wrappers"] = pantheon.patcher_extension.copy_nested_dicts(model.wrappers)
    model_options["transformer_options"]["callbacks"] = pantheon.patcher_extension.copy_nested_dicts(model.callbacks)
    # begin registering hooks
    registered = pantheon.hooks.HookGroup()
    target_dict = pantheon.hooks.create_target_dict(pantheon.hooks.EnumWeightTarget.Model)
    # handle all TransformerOptionsHooks
    for hook in hooks.get_type(pantheon.hooks.EnumHookType.TransformerOptions):
        hook: pantheon.hooks.TransformerOptionsHook
        hook.add_hook_patches(model, model_options, target_dict, registered)
    # handle all AddModelsHooks
    for hook in hooks.get_type(pantheon.hooks.EnumHookType.AdditionalModels):
        hook: pantheon.hooks.AdditionalModelsHook
        hook.add_hook_patches(model, model_options, target_dict, registered)
    ...
    for wc_name in ["wrappers", "callbacks"]:
        pantheon.patcher_extension.merge_nested_dicts(to_load_options.setdefault(wc_name, {}), model_options["transformer_options"][wc_name],
                                                    copy_dict1=False)
