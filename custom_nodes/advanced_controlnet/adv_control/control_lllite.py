# adapted from https://github.com/kohya-ss/ControlNet-LLLite-PantheonUI
...
# based on set_model_patch code in pantheon/model_patcher.py
...
# TODO: use pantheon.ops to support fp8 properly
    ...
    def forward(self, x: Tensor, control: Union[AdvancedControlBase, ControlBase]):
        ...
        if self.cond_emb is None or control.sub_idxs != self.prev_sub_idxs or x.shape[0] != self.prev_batch:
            ...
            if control.latent_dims_div2 is not None and x.shape[-1] != 1280:
                cond_hint = pantheon.utils.common_upscale(cond_hint, control.latent_dims_div2[0] * 8, control.latent_dims_div2[1] * 8, 'nearest-exact', "center").to(x.device, dtype=x.dtype)
            elif control.latent_dims_div4 is not None and x.shape[-1] == 1280:
                cond_hint = pantheon.utils.common_upscale(cond_hint, control.latent_dims_div4[0] * 8, control.latent_dims_div4[1] * 8, 'nearest-exact', "center").to(x.device, dtype=x.dtype)
...
class ControlLLLiteAdvanced(ControlBase, AdvancedControlBase):
    ...
    def __init__(self, patch_attn1: LLLitePatch, patch_attn2: LLLitePatch, timestep_keyframes: TimestepKeyframeGroup, device, ops: pantheon.ops.disable_weight_init):
        super().__init__()
        ...
        self.control_model_wrapped = ModelPatcher(self.control_model, load_device=device, offload_device=pantheon.model_management.unet_offload_device())
...
def load_controllllite(ckpt_path: str, controlnet_data: dict[str, Tensor]=None, timestep_keyframe: TimestepKeyframeGroup=None):
    ...
    ops = pantheon.ops.disable_weight_init
    if manual_cast_dtype is not None:
        ops = pantheon.ops.manual_cast
