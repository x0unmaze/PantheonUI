...
import pantheon.controlnet as pantheon_cn
from pantheon.controlnet import ControlBase, ControlNet, ControlNetSD35, ControlLora, T2IAdapter, StrengthType
...
from .utils import (
    AdvancedControlBase, TimestepKeyframeGroup, LatentKeyframeGroup,
    AbstractPreprocWrapper, ControlWeightType, ControlWeights,
    WeightTypeException, Extras, manual_cast_clean_groupnorm,
    disable_weight_init_clean_groupnorm, WrapperConsts,
    prepare_mask_batch, get_properly_arranged_t2i_weights,
    load_torch_file_with_dict_factory, broadcast_image_to_extend, extend_to_batch_size,
    ORIG_PREVIOUS_CONTROLNET, CONTROL_INIT_BY_ACN,
)
...
class ControlNetAdvanced(ControlNet, AdvancedControlBase):
    def __init__(
        self,
        control_model,
        timestep_keyframes: TimestepKeyframeGroup,
    ...
    def sliding_get_control(self, x_noisy: Tensor, t, cond, batched_number, transformer_options):
        ...
        if self.sub_idxs is not None or self.cond_hint is None or x_noisy.shape[2] * self.real_compression_ratio != self.cond_hint.shape[2] or x_noisy.shape[3] * self.real_compression_ratio != self.cond_hint.shape[3]:
            ...
            if self.sub_idxs is not None:
                ...
                self.cond_hint = pantheon.utils.common_upscale(actual_cond_hint_orig[self.sub_idxs], x_noisy.shape[3] * compression_ratio, x_noisy.shape[2] * compression_ratio, self.upscale_algorithm, "center")
            else:
...
class T2IAdapterAdvanced(T2IAdapter, AdvancedControlBase):
    ...
    def get_calc_pow(self, idx: int, control: dict[str, list[Tensor]], key: str) -> int:
        ...
        if key == "input":
            indeces.reverse()  # need to reverse to match recent PantheonUI changes
        return indeces[idx]
    ...
    def get_control_advanced(self, x_noisy, t, cond, batched_number, transformer_options):
        ...
        if self.sub_idxs is not None or self.cond_hint is None or x_noisy.shape[2] * 8 != self.cond_hint.shape[2] or x_noisy.shape[3] * 8 != self.cond_hint.shape[3]:
            ...
            if self.sub_idxs is not None:
                ...
                if self.cond_hint_original.size(0) < self.full_latent_length:
                    actual_cond_hint_orig = extend_to_batch_size(tensor=actual_cond_hint_orig, batch_size=self.full_latent_length)
                self.cond_hint = pantheon.utils.common_upscale(actual_cond_hint_orig[self.sub_idxs], x_noisy.shape[3] * 8, x_noisy.shape[2] * 8, 'nearest-exact', "center").to(dtype).to(x_noisy.device)
            else:
                self.cond_hint = pantheon.utils.common_upscale(self.cond_hint_original, x_noisy.shape[3] * 8, x_noisy.shape[2] * 8, 'nearest-exact', "center").to(dtype).to(x_noisy.device)
        if x_noisy.shape[0] != self.cond_hint.shape[0]:
            self.cond_hint = broadcast_image_to_extend(self.cond_hint, x_noisy.shape[0], batched_number)
        ...
        context = cond.get('crossattn_controlnet', cond['c_crossattn'])
        # uses 'y' in new PantheonUI update
        y = cond.get('y', None)
        if y is not None:
            y = y.to(dtype)
        timestep = self.model_sampling_current.timestep(t)
...
class SparseCtrlAdvanced(ControlNetAdvanced):
    def __init__(self, control_model: SparseControlNet, motion_model: InterfaceAnimateDiffModel,
        ...
        if control_model is not None:
            self.control_model_wrapped: ModelPatcher = create_sparse_modelpatcher(self.control_model, motion_model, load_device=load_device, offload_device=pantheon.model_management.unet_offload_device())
    ...
    def get_control_advanced(self, x_noisy: Tensor, t, cond, batched_number: int, transformer_options):
        ...
        if self.sub_idxs is not None or self.cond_hint is None or x_noisy.shape[2]*dim_mult != self.cond_hint.shape[2] or x_noisy.shape[3]*dim_mult != self.cond_hint.shape[3]:
            ...
            if self.control_model.use_simplified_conditioning_embedding:
                ...
                sub_cond_hint = pantheon.utils.common_upscale(sub_cond_hint, x_noisy.shape[3], x_noisy.shape[2], "nearest-exact", "center").to(dtype).to(x_noisy.device)
            else:
                # other SparseCtrl; inputs are typical images
                sub_cond_hint = pantheon.utils.common_upscale(sub_cond_hint, x_noisy.shape[3] * 8, x_noisy.shape[2] * 8, 'nearest-exact', "center").to(dtype).to(x_noisy.device)
...
def load_controlnet(ckpt_path, timestep_keyframe: TimestepKeyframeGroup=None, model=None):
    controlnet_data = pantheon.utils.load_torch_file(ckpt_path, safe_load=True)
    ...
    # otherwise, load vanilla ControlNet
        ...
        try:
            ...
            orig_load_torch_file = pantheon.utils.load_torch_file
            pantheon.utils.load_torch_file = load_torch_file_with_dict_factory(controlnet_data, orig_load_torch_file)
            control = pantheon_cn.load_controlnet(ckpt_path, model=model)
        finally:
            pantheon.utils.load_torch_file = orig_load_torch_file
    if control is None:
...
def load_sparsectrl(ckpt_path: str, controlnet_data: dict[str, Tensor]=None, timestep_keyframe: TimestepKeyframeGroup=None, sparse_settings=SparseSettings.default(), model=None) -> SparseCtrlAdvanced:
    ...
    # now, load as if it was a normal controlnet - mostly copied from pantheon load_controlnet function
    ...
    if is_diffusers: #diffusers format
        ...
        controlnet_config = pantheon.model_detection.unet_config_from_diffusers_unet(controlnet_data, unet_dtype)
    ...
    if controlnet_config is None:
        unet_dtype = pantheon.model_management.unet_dtype()
        controlnet_config = pantheon.model_detection.model_config_from_unet(controlnet_data, prefix, unet_dtype, True).unet_config
    ...
    motion_model = load_sparsectrl_motionmodel(ckpt_path=ckpt_path, motion_data=motion_data, ops=controlnet_config.get("operations", None)).to(pantheon.model_management.unet_dtype())
...
def load_svdcontrolnet(ckpt_path: str, controlnet_data: dict[str, Tensor]=None, timestep_keyframe: TimestepKeyframeGroup=None, model=None):
    ...
    manual_cast_dtype = pantheon.model_management.unet_manual_cast(unet_dtype, load_device)
    if manual_cast_dtype is not None:
        controlnet_config["operations"] = pantheon.ops.manual_cast
    controlnet_config.pop("out_channels")
    controlnet_config["hint_channels"] = controlnet_data["{}input_hint_block.0.weight".format(prefix)].shape[1]
