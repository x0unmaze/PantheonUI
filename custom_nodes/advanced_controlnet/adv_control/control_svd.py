...
from pantheon.utils import UNET_MAP_BASIC, UNET_MAP_RESNET, UNET_MAP_ATTENTIONS, TRANSFORMER_BLOCKS
...
from pantheon.ldm.modules.attention import SpatialVideoTransformer
from pantheon.ldm.modules.diffusionmodules.openaimodel import UNetModel, TimestepEmbedSequential, VideoResBlock, Downsample 
from pantheon.ldm.util import exists
import pantheon.ops
...
class SVDControlNet(nn.Module):
    def __init__(
        ...
        video_kernel_size=3,
        device=None,
        operations=pantheon.ops.disable_weight_init,
        **kwargs,
    ):
        super().__init__()
...
def svd_unet_config_from_diffusers_unet(state_dict: dict[str, Tensor], dtype):
    ...
    attn_res = 1
    down_blocks = pantheon.model_detection.count_blocks(state_dict, "down_blocks.{}")
    for i in range(down_blocks):
        attn_blocks = pantheon.model_detection.count_blocks(state_dict, "down_blocks.{}.attentions.".format(i) + '{}')
        for ab in range(attn_blocks):
            transformer_count = pantheon.model_detection.count_blocks(state_dict, "down_blocks.{}.attentions.{}.transformer_blocks.".format(i, ab) + '{}')
    ...
    for unet_config in supported_models:
        ...
        for k in match:
            if match[k] != unet_config[k]:
                ...
                break
        if matches:
            return pantheon.model_detection.convert_config(unet_config)
    return None
