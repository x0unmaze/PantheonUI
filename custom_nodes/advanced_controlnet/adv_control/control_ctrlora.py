...
from pantheon.ldm.modules.diffusionmodules.util import (
    zero_module,
    timestep_embedding,
)
...
def load_ctrlora(base_path: str, lora_path: str,
    ...
    if base_data is None:
        base_data = pantheon.utils.load_torch_file(base_path, safe_load=True)
...
def load_lora_data(control: CtrLoRAAdvanced, lora_path: str, loaded_data: dict[str, Tensor]=None, lora_strength=1.0):
    if loaded_data is None:
        loaded_data = pantheon.utils.load_torch_file(lora_path, safe_load=True)
    ...
    for down_key in data_lora:
        ...
        # currently, PantheonUI expects 6 elements in 'lora' type, but for future-proofing add a bunch more with None
