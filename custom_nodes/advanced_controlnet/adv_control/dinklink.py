...
#
# My DinkLink-compatible nodes will inject pantheon.hooks with a __DINKLINK attr
...
# at any time.
#
# DinkLink also serves as a proof-of-concept for a future PantheonUI implementation of
...
def create_dinklink():
    if not hasattr(pantheon.hooks, DINKLINK):
        setattr(pantheon.hooks, DINKLINK, {})

def get_dinklink() -> dict[str, dict[str]]:
    create_dinklink()
    return getattr(pantheon.hooks, DINKLINK)
