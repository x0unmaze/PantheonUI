...
from pantheon.ldm.modules.diffusionmodules.util import (zero_module, timestep_embedding)

from pantheon.cldm.cldm import ControlNet as ControlNetCLDM
import pantheon.cldm.cldm
from pantheon.controlnet import ControlNet
#from pantheon.t2i_adapter.adapter import ResidualAttentionBlock
from pantheon.ldm.modules.attention import optimized_attention
import pantheon.ops
...
# parts taken from pantheon/cldm/cldm.py
...
class ControlAddEmbeddingAdv(nn.Module):
    def __init__(self, in_dim, out_dim, num_control_type, dtype=None, device=None, operations: pantheon.ops.disable_weight_init=None):
        super().__init__()
...
class ControlNetPlusPlus(ControlNetCLDM):
    def __init__(self, *args,**kwargs):
        ...
        operations: pantheon.ops.disable_weight_init = kwargs.get("operations", pantheon.ops.disable_weight_init)
        ...
        if self.sub_idxs is not None or self.cond_hint is None or x_noisy.shape[2] * self.compression_ratio != self.cond_hint_shape[2] or x_noisy.shape[3] * self.compression_ratio != self.cond_hint_shape[3]:
            ...
            for pp_type, pp_input in self.cond_hint_original.controls.items():
                ...
                if self.sub_idxs is not None:
                    ...
                    self.cond_hint[pp_idx] = pantheon.utils.common_upscale(actual_cond_hint_orig[self.sub_idxs], x_noisy.shape[3] * compression_ratio, x_noisy.shape[2] * compression_ratio, 'nearest-exact', "center")
                else:
                    self.cond_hint[pp_idx] = pantheon.utils.common_upscale(pp_input.image, x_noisy.shape[3] * compression_ratio, x_noisy.shape[2] * compression_ratio, 'nearest-exact', "center")
...
def load_controlnetplusplus(ckpt_path: str, timestep_keyframe: TimestepKeyframeGroup=None, model=None):
    ...
    if supported_inference_dtypes is None:
        ...
        unet_dtype = pantheon.model_management.unet_dtype(supported_dtypes=supported_inference_dtypes)
