...
class disable_weight_init_clean_groupnorm(pantheon.ops.disable_weight_init):
    class GroupNorm(pantheon.ops.disable_weight_init.GroupNorm):
...
class AdvancedControlBase:
    ...
    def set_cond_hint_inject(self, *args, **kwargs):
        ...
        if self.require_vae:
            # check args first, as that's the default way vae param is used in PantheonUI
            for arg in args:
                    ...
                    if i >= len(o):
                        ...
                        if o[i] is None:
                            ...
                            if o[i].shape[0] < prev_val.shape[0]:
                                ...
                                o[i] = prev_val + o[i]  # TODO from base PantheonUI: change back to inplace add if shared tensors stop being an issue
        return out
