...
def prepare_dinklink_acn_wrapper():
    ...
    link_acn[WrapperConsts.ACN_CREATE_SAMPLER_SAMPLE_WRAPPER] = (pantheon.patcher_extension.WrappersMP.OUTER_SAMPLE,
...
def should_register_outer_sample_wrapper(hook, model, model_options: dict, target, registered: list):
    wrappers = pantheon.patcher_extension.get_wrappers_with_key(pantheon.patcher_extension.WrappersMP.OUTER_SAMPLE,
                                                  WrapperConsts.ACN_OUTER_SAMPLE_WRAPPER_KEY,
...
def create_wrapper_hooks():
    wrappers = {}
    pantheon.patcher_extension.add_wrapper_with_key(pantheon.patcher_extension.WrappersMP.OUTER_SAMPLE,
                                                 WrapperConsts.ACN_OUTER_SAMPLE_WRAPPER_KEY,
                                                 acn_outer_sample_wrapper,
                                                 transformer_options=wrappers)
    hooks = pantheon.hooks.HookGroup()
    hook = pantheon.hooks.WrapperHook(wrappers)
...
def acn_outer_sample_wrapper(executor, *args, **kwargs):
    ...
    guider: pantheon.samplers.CFGGuider = executor.class_obj
    ...
    try:
        ...
        if has_sliding_context_windows(guider.model_patcher):
            new_model_options = pantheon.model_patcher.create_model_options_clone(new_model_options)
