...
from pantheon.ldm.modules.attention import BasicTransformerBlock
from pantheon.ldm.modules.diffusionmodules import openaimodel

from .logger import logger
from .utils import (
    AdvancedControlBase, ControlWeights,
    TimestepKeyframeGroup, TimestepKeyframe, AbstractPreprocWrapper,
...
class ReferenceAdvanced(ControlBase, AdvancedControlBase):
    ...
    def __init__(self, ref_opts: ReferenceOptions, timestep_keyframes: TimestepKeyframeGroup, extra_hooks: pantheon.hooks.HookGroup=None):
        super().__init__()
    ...
    def get_control_advanced(self, x_noisy: Tensor, t, cond, batched_number: int, transformer_options):
        ...
        if self.cond_hint_original is not None:
            ...
            if self.sub_idxs is not None and self.cond_hint_original.size(0) >= self.full_latent_length:
                self.cond_hint = pantheon.utils.common_upscale(
...
def handle_reference_injection(model_options: dict, reference_injections: ReferenceInjections):
    ...
    pantheon.patcher_extension.add_wrapper_with_key(pantheon.patcher_extension.WrappersMP.DIFFUSION_MODEL,
    ...
    def refcn_diffusion_model_wrapper(executor, x, *args, **kwargs):
        ...
        try:
            ...
            if len(adain_controlnets) > 0 or len(context_adain_controlnets) > 0:
                # PantheonUI uses forward_timestep_embed with the TimestepEmbedSequential passed into it
...
def _forward_inject_BasicTransformerBlock(self: RefBasicTransformerBlock, x: Tensor, context: Tensor=None, transformer_options: dict[str]={}):
    ...
    if len(cref_write_cns) > 0:
        ...
        for refcn in cref_write_cns:
            ...
            self.injection_holder.bank_styles.set_c_bank_for_uuids(cached_n.to(pantheon.model_management.unet_offload_device()), uuids)
