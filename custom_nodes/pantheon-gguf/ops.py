...
class GGMLTensor(torch.Tensor):
    ...
    def copy_(self, *args, **kwargs):
        # fixes .weight.copy_ in pantheon/clip_model/CLIPTextModel
        try:
...
class GGMLLayer(torch.nn.Module):
    ...
    def cast_bias_weight(s, input=None, dtype=None, device=None, bias_dtype=None):
        ...
        if s.bias is not None:
            ...
            bias = pantheon.ops.cast_to(bias, bias_dtype, device, non_blocking=non_blocking, copy=False)

        weight = s.get_weight(s.weight.to(device), dtype)
        weight = pantheon.ops.cast_to(weight, dtype, device, non_blocking=non_blocking, copy=False)
        return weight, bias

    def forward_pantheon_cast_weights(self, input, *args, **kwargs):
        if self.is_ggml_quantized():
            out = self.forward_ggml_cast_weights(input, *args, **kwargs)
        else:
            out = super().forward_pantheon_cast_weights(input, *args, **kwargs)
...
class GGMLOps(pantheon.ops.manual_cast):
    """
    Dequantize weights on the fly before doing the compute
    """
    class Linear(GGMLLayer, pantheon.ops.manual_cast.Linear):
    ...
    class Conv2d(GGMLLayer, pantheon.ops.manual_cast.Conv2d):
        def forward_ggml_cast_weights(self, input):
            weight, bias = self.cast_bias_weight(input)
            return self._conv_forward(input, weight, bias)

    class Embedding(GGMLLayer, pantheon.ops.manual_cast.Embedding):
    ...
    class LayerNorm(GGMLLayer, pantheon.ops.manual_cast.LayerNorm):
        def forward_ggml_cast_weights(self, input):
            if self.weight is None:
                return super().forward_pantheon_cast_weights(input)
            weight, bias = self.cast_bias_weight(input)
            return torch.nn.functional.layer_norm(input, self.normalized_shape, weight, bias, self.eps)

    class GroupNorm(GGMLLayer, pantheon.ops.manual_cast.GroupNorm):
        def forward_ggml_cast_weights(self, input):
            weight, bias = self.cast_bias_weight(input)
