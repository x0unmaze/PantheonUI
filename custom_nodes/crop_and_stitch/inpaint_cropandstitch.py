...
class InpaintCrop:
    """
    PantheonUI-InpaintCropAndStitch
    https://github.com/lquesada/PantheonUI-InpaintCropAndStitch
...
class InpaintStitch:
    ...
    # This function is from pantheon_extras: https://github.com/pantheonanonymous/PantheonUI
    def composite(self, destination, source, x, y, mask=None, multiplier=8, resize_source=False):
        source = source.to(destination.device)
        if resize_source:
            source = torch.nn.functional.interpolate(source, size=(destination.shape[2], destination.shape[3]), mode="bilinear")

        source = pantheon.utils.repeat_to_batch_size(source, destination.shape[0])
        ...
        if mask is None:
            ...
            mask = pantheon.utils.repeat_to_batch_size(mask, source.shape[0])
    ...
    def inpaint_stitch(self, stitch, inpainted_image, rescale_algorithm: str = 'bislerp'):
        results = []
...
class InpaintExtendOutpaint:
    ...
    def inpaint_extend(self, image, mask, mode='factors', expand_up_pixels=0, expand_up_factor=1.0, expand_down_pixels=0, expand_down_factor=1.0, expand_left_pixels=0, expand_left_factor=1.0, expand_right_pixels=0, expand_right_factor=1.0, optional_context_mask=None):
        assert image.shape[0] == mask.shape[0], "Batch size of images and masks must be the same"
        if optional_context_mask is not None:
            assert optional_context_mask.shape[0] == image.shape[0], "Batch size of optional_context_masks must be the same as images or None"
