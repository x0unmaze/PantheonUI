...
// Some fragments of this code are from https://github.com/LucianoCirino/efficiency-nodes-pantheonui

function inpaintCropAndStitchHandler(node) {
  if (node.pantheonClass == "InpaintCrop") {
  ...
  } else if (node.pantheonClass == "InpaintExtendOutpaint") {
  ...
  } else if (node.pantheonClass == "InpaintResize") {
    toggleWidget(node, findWidgetByName(node, "min_width"))
    toggleWidget(node, findWidgetByName(node, "min_height"))
    toggleWidget(node, findWidgetByName(node, "rescale_factor"))
